    ...
    if (interaction.user.id == '553337834090659899') {
      ...
      try {
        ...
        try {
          ...
          console.log(((endTimeMinutes - startTimeMinutes)/1000)/60)  

          const loadTime = ((endTimeMinutes - startTimeMinutes)/1000)/60

          await interaction.followUp({ embeds: [createdEmbed('5beb34', `Emojis and Messages are up to date now`, `The load time took ${loadTime.toFixed(2)} minutes`)], ephemeral: false })
        ...
        await interaction.reply({ embeds: [createdEmbed('FF0000', `There was an error with the load command`)], ephemeral: false });
      }
    } else {
      await interaction.reply({ embeds: [createdEmbed('FF0000', `You are not allowed to use this command`)], ephemeral: false });
    }
  },
};
